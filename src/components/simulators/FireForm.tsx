type Props = {
  depensesAnnuelles: string;
  tauxRetrait: string;
  capitalActuel: string;
  versementMensuel: string;
  tauxRendementAnnuel: string;
  ageActuel: string;
  onDepensesAnnuellesChange: (v: string) => void;
  onTauxRetraitChange: (v: string) => void;
  ...
  return (
    <div className="bg-zinc-900 rounded-3xl p-8 shadow-lg mt-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          <label className="text-zinc-400">Dépenses annuelles visées</label>

          <div className="relative mt-2">
            <input
              type="number"
              value={depensesAnnuelles}
              onChange={(e) => onDepensesAnnuellesChange(e.target.value)}
        ...
        <div>
          <label className="text-zinc-400">
            Taux de retrait{" "}
            <span className="text-zinc-600">(4% = règle classique)</span>
          </label>
          ...
          <div className="relative mt-2">
            <input
              ...
              step="0.1"
              value={tauxRetrait}
              onChange={(e) => onTauxRetraitChange(e.target.value)}
              className="w-full rounded-xl bg-zinc-800 p-3 pr-10"
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500">
              %
